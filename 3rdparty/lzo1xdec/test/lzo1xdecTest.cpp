...
CXXTEST_SUITE(LZO1xDecTest)
{
  CXXTEST_TEST(decode)
  {
    ...
    int remlen = outlen;
    ::lzo1x_decode(out, &remlen, _lzo_test_encoded, &inlen);
    TS_ASSERT_EQUALS(remlen, 0); // All the buffer was used
    TS_ASSERT_EQUALS(memcmp(out, _lzo_test_decoded, outlen), 0);
  };
};

CXXTEST_SUITE_REGISTRATION(LZO1xDecTest)
CXXTEST_TEST_REGISTRATION(LZO1xDecTest, decode)
